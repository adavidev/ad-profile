...
(function() {
  ...
  ctrl = app.controller("MainController", [
    "$scope", "$resource", "$sce", "Post", "Activity", function($scope, $resource, $sce, Post, Activity) {
      $scope.posts = Post.query(function() {
        return $scope.select($scope.posts[0]);
      });
      $scope.select = function(post) {
        $scope.selected_post = post;
        return Activity.store(post);
      };
      return $scope.oldPost = function(post) {
        return Activity.get().filter(function(obj) {
    ...
    function() {
      return {
        ...
        store: function(json) {
          ...
          actions.push(json);
          return localStorage.setItem("current-session", JSON.stringify(actions));
        },
        get: function() {
